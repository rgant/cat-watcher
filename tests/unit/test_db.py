...
def test_round_trip_camera_clip_alert(db_engine: Engine) -> None:
...
def test_clip_unique_camera_source_filename(db_engine: Engine) -> None:
    ...
    with pytest.raises(IntegrityError), get_session(db_engine) as session:
        session.add(duplicate)


def test_get_session_rolls_back_on_exception(db_engine: Engine) -> None:
...
def test_camera_cascade_deletes_clips(db_engine: Engine) -> None:
